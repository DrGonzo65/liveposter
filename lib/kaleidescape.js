...
class KaleidescapeClient extends EventEmitter {
  ...
  parseResponse(response) {
    ...
    if (!noisy.includes(responseName)) {
    ...
    if (statusCode && statusCode !== '000' && statusCode !== '!' && !/^[0-9]$/.test(statusCode)) {
      ...
      if (this.lastError !== response) {
        ...
        console.log(`Kaleidescape error [${statusCode}] ${responseName}${rejectedCommand ? ` (in reply to ${rejectedCommand})` : ''}: ${parts.slice(2).join(':')}`);
      }

      // A movie server (Terra) accepts library queries but has no playback zone,
      // so it rejects these. Without this hint the symptom is silent: the
      // slideshow works forever and "Now Playing" never appears.
      const playbackCommands = ['GET_PLAY_STATUS', 'GET_PLAYING_TITLE_NAME'];
      if (playbackCommands.includes(rejectedCommand) && !this.playbackUnsupported) {
    ...
    switch (responseName) {
      ...
      default:
        if (responseName && !responseName.startsWith('Invalid')) {
          console.log(`Kaleidescape unhandled [${responseName}]:`, parts.slice(2).join(':'));
        }
    }
  }
  ...
  handlePlayStatus(parts) {
    ...
    console.log(`Kaleidescape PLAY_STATUS: ${playStatus} (${this.currentState.playing ? 'active' : 'stopped'})`);
  ...
  handleMovieLocation(parts) {
    ...
    console.log(`Kaleidescape movie location: ${location}`);
  }
