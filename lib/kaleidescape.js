...
class KaleidescapeClient extends EventEmitter {
  ...
  parseResponse(response) {
    ...
    if (statusCode && statusCode !== '000' && statusCode !== '!' && !/^[0-9]$/.test(statusCode)) {
      ...
      if (this.lastError !== response) {
        ...
        console.log(`Kaleidescape error [${statusCode}] ${responseName}: ${parts.slice(2).join(':')}`);
      }
      return;
    }
  ...
  async loadMovieLibrary(httpHost) {
    if (!httpHost) httpHost = this.host;

    console.log('Loading Kaleidescape library from HTTP interface...');
    const axios = require('axios');

    try {
      const response = await axios.get(`http://${httpHost}/movies`, { timeout: 10000 });
