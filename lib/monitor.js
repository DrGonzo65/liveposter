...
class MediaMonitor extends EventEmitter {
  ...
  initializeClients() {
    // Kaleidescape
    if (this.config.kaleidescape?.playerHost) {
  ...
  rebuildCombinedLibrary() {
    ...
    // twice. Keep one copy per title, preferring the better-sourced entry.
    const best = new Map();
    for (const movie of combined) {
      ...
      const better = isEnriched(movie) !== isEnriched(held)
        ? isEnriched(movie)
        : (SOURCE_PRIORITY[movie.source] ?? 99) < (SOURCE_PRIORITY[held.source] ?? 99);

      if (better) best.set(key, movie);
    }
    ...
    this.allLibraries = [...best.values()].sort((a, b) =>
      sortKey(a.title).localeCompare(sortKey(b.title), undefined, {
